...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.info(
        "College Football version %s is starting, if you have any issues please report them here: %s",
        VERSION,
        ISSUE_URL,
    )
...
class CollegeFootballDataUpdateCoordinator(DataUpdateCoordinator):
    """Class to manage fetching College Football data."""
            ...
            if team_id in event["shortName"]:
                ...
                if event["status"]["type"]["state"].lower() in [
                    ...
                    if (
                        ...
                        try: values["team_timeouts"] = event["competitions"][0]["situation"][
                            "homeTimeouts"
                        ]
                        except: values["team_timeouts"] = None
                        
                        try: values["opponent_timeouts"] = event["competitions"][0][
                            "situation"
                        ]["awayTimeouts"]
                        except: values["opponent_timeouts"] = None
                        try:
